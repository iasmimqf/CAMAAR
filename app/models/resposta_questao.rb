class RespostaQuestao < ApplicationRecord
  ...
  serialize :valor_resposta, coder: JSON # atualizado para evitar warning
  ...
  def resposta_adequada_ao_tipo
    ...
    case questao.tipo
    when 'Escala'
      unless resposta_valor.is_a?(Array) && resposta_valor.size == 1 && resposta_valor.first.is_a?(Integer)
        errors.add(:base, "A resposta para a questão '#{questao.enunciado}' deve ser uma única opção.")
        return
      end

      valor_numerico = resposta_valor.first
      max_opcao_index = questao.opcoes_array.size - 1

      unless valor_numerico.between?(0, max_opcao_index)
        errors.add(:base, "A opção '#{valor_numerico + 1}' não é válida para a questão '#{questao.enunciado}'.")
      end
